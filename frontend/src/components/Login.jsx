...
const Login = ({ onLogin, darkMode, redirectTo = '/dashboard' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.message && (error.message.toLowerCase().includes('inactive') || error.message.toLowerCase().includes('deactivated'))) {
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 pr-11 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${
                    darkMode 
                      ? 'bg-slate-700 border-slate-600 text-white placeholder-slate-400' 
                      : 'bg-white border-slate-300 text-slate-900 placeholder-slate-500'
                  }`}
