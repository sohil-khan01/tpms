...
import { HiEnvelope, HiPhone, HiBuildingOffice2, HiMapPin, HiCalendar, HiUsers, HiCalendarDays, HiCheckCircle, HiBolt } from 'react-icons/hi2';
...
const AdminProfile = ({ darkMode, adminUser }) => {
  ...
  const [adminData, setAdminData] = useState({
    ...
    role: 'Loading...',
    department: 'Loading...',
    company: '8Bit System Private Limited',
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        setAdminData({
          ...
          company: adminUser.company || '8Bit System Private Limited',
          createdAt: adminUser.createdAt || adminUser.joinDate || new Date().toISOString().split('T')[0],
          joinDate: adminUser.createdAt || adminUser.joinDate || new Date().toISOString().split('T')[0],
          location: adminUser.location || 'Jaipur',
        });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        company: adminData.company?.trim() || null,
        location: adminData.location?.trim() || null,
        joinDate: adminData.createdAt?.trim() || adminData.joinDate?.trim() || null,
  ...
  return (
    <div className="p-8">
      ...
      {/* Loading State */}
      {loading && (
        <InlineLoader message="Loading user profile..." darkMode={darkMode} />
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className={`lg:col-span-1 ${darkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl border ${darkMode ? 'border-slate-700' : 'border-slate-200'} shadow-sm p-6`}>
            ...
            <div className="space-y-3">
              ...
              <div className={`flex items-center gap-3 text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                <HiBuildingOffice2 className="w-5 h-5 flex-shrink-0" />
                <span>{adminData.company}</span>
              </div>
              <div className={`flex items-center gap-3 text-sm ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                <HiMapPin className="w-5 h-5 flex-shrink-0" />
            ...
            <button
              ...
              disabled={loading}
              className={`w-full mt-6 py-2 rounded-lg transition-colors cursor-pointer ${loading
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {isEditing && (
                <button
                  onClick={handleSave}
                  disabled={loading}
                  className={`px-4 py-2 rounded-lg transition-colors cursor-pointer ${loading
                    ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                ...
                >
                  {loading ? (
                    <div className="flex items-center gap-2">
                      <InlineLoader size="sm" darkMode={darkMode} />
                      Saving...
                    </div>
                  ) : (
                    'Save Changes'
              ...
              <div>
                ...
                {isEditing ? (
                  <input
                    ...
                    required
                    className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'
                      }`}
              ...
              <div>
               ...
               <div>
                ...
                {isEditing ? (
                  <input
                    type="date"
                    value={adminData.createdAt ? new Date(adminData.createdAt).toISOString().split('T')[0] : ''}
                    onChange={(e) => setAdminData({ ...adminData, createdAt: e.target.value })}
                    className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-white border-slate-300'
                      }`}
                  />
                ) : (
                  <p className={`px-4 py-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                    {adminData.createdAt ? new Date(adminData.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Not set'}
                  </p>
                )}
              </div>
            </div>
