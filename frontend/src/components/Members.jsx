...
import { membersAPI, authAPI, handleAPIError } from '../utils/api';
import { HiMagnifyingGlass, HiXCircle, HiUserPlus, HiEye, HiEyeSlash } from 'react-icons/hi2';
...
const Members = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    authAPI.checkAuthorities().then(authorities => {
      console.log('Members component - Current user authorities:', authorities);
    }).catch(err => {
      console.error('Members component - Failed to check authorities:', err);
    });
  ...
  const handleToggleStatus = async (member) => {
    ...
    const confirmed = window.confirm(`Are you sure you want to ${action} ${member.username}?`);
    if (!confirmed) return;

    try {
      setLoading(true);
      if (newStatus === 'INACTIVE') {
        // This sends WebSocket logout signal + updates status
  ...
  const handleDeleteMember = async (id) => {
    ...
    const confirmed = window.confirm(`Are you sure you want to delete ${memberToDelete.username}? This cannot be undone.`);
    if (!confirmed) return;

    try {
      setLoading(true);
      // softDelete sends WebSocket logout signal before deleting
  ...
  if (loading) {
    return <PageLoader message="Loading members..." darkMode={darkMode} />;
  }
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-4 py-2 pr-11 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                  ...
                  >
                    {showMemberPassword ? <HiEyeSlash className="w-4 h-4" /> : <HiEye className="w-4 h-4" />}
                  </button>
                </div>
                {validationErrors.password && (
